...
def prototypical_triplet_loss(labels, embeddings, squared=True, margin=1.0):
	...
	Args:
		...
		squared: bool, whether to use squared distances or not
		margin: float, the margin of the triplet loss
	Returns:
		triplet_loss: tf.float32 scalar.
	"""

	# get cluster labels
	clusters_labels, _, num_embeddings_per_cluster = tf.unique_with_counts(labels)
	...
	adjacency = tf.equal(labels, tf.transpose(clusters_labels)) # [batch_size, num_clusters]

	# computes the centroids embedding
	centroids = tf.matmul(tf.cast(adjacency, dtype=tf.float32), embeddings, transpose_a=True) # [num_clusters, embeddings_size]
	centroids = tf.div(centroids, tf.expand_dims(tf.cast(num_embeddings_per_cluster, dtype=tf.float32), axis=1))
	centroids = tf.nn.l2_normalize(centroids, axis=1)

	# Build pairwise squared distance matrix.
	pairwise_distances = _pairwise_distances(feature_A=embeddings, feature_B=centroids, squared=squared) # [batch_size, num_clusters]
	...
	mask = tf.logical_and(
		...
		tf.greater(
			pairwise_distances_tile, tf.reshape(
				tf.transpose(pairwise_distances), [-1, 1]))) # [bxn, n]
	mask_final = tf.reshape(
		tf.greater(
			tf.reduce_sum(
				tf.cast(mask, dtype=tf.float32), 1, keepdims=True),
			0.0), [num_clusters, batch_size])
	...
	mask_positives = tf.cast(
		adjacency, dtype=tf.float32) # - tf.diag(tf.ones([batch_size]))
...
def _pairwise_distances(feature_A, feature_B=None, squared=False):
	...
	pairwise_distances_squared = tf.add(
		...
		tf.reduce_sum(
			tf.square(tf.transpose(feature_B)),
			axis=[0],
			keepdims=True)) - 2.0 * tf.matmul(feature_A, tf.transpose(feature_B))
	...
	if squared:
		...
		pairwise_distances = tf.sqrt(
			pairwise_distances_squared + tf.to_float(error_mask) * 1e-16)

	# Undo conditionally adding 1e-16.
	pairwise_distances = tf.multiply(
		pairwise_distances, tf.to_float(tf.logical_not(error_mask)))

	if feature_B is None:
		num_data = tf.shape(feature_A)[0]
		# Explicitly set diagonals to zero.
		mask_offdiagonals = tf.ones_like(pairwise_distances) - tf.diag(
			tf.ones([num_data]))
